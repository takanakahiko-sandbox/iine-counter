<template>
  <div class="container">
    ...
    <div class="note">
      メンテナンス用(最初にカウンタを初期化するやつ)→
      <input type="button" value="create" @click="createCounter">
    </div>
...
export default Vue.extend({
  ...
  methods: {
    increment() {
      ...
      const shard_ref = db.collection('counters').doc(shard_id);
      return shard_ref.update("count", firebase.firestore.FieldValue.increment(1));
    },
    createCounter() { // メンテナンス用(最初にカウンタを初期化するやつ)
      var batch = db.batch();
      for (let i = 0; i < num_shards; i++) {
          let shardRef = db.collection('counters').doc(i.toString());
